...
void addFaculty(faculty members[], faculty member) {
  ...
  while (c < 3) {
    printf("\tID: ");
    if (scanf("%2s%d", member.nid, &member.id) != 2 ||
        strlen(member.nid) != 2) {
  ...
  while (c < 3) {
    ...
    if (strcmp(member.rank, "PROFESSOR") != 0 &&
        strcmp(member.rank, "ADJUNCT") != 0) {
  ...
  while (c < 3) {
    ...
    if (strcmp(member.department, "MATH") != 0 &&
        strcmp(member.department, "CS") != 0 &&
        strcmp(member.department, "SCIENCE") != 0) {
...
void printInvoice(student studs[]) {
  ...
  for (int i = 0; i < SSIZE; i++) {
    ...
    if (studs[i].id != 0 && strcmp(studs[i].nid, nid2) == 0 ||
        strcmp(nid2, tempnid2) == 0 && studs[i].id == id2) {
      found = 1;
...
void printFaculty(faculty members[]) {
  ...
  for (int i = 0; i < FSIZE; i++) {
    ...
    if (members[i].id != 0 && strcmp(members[i].nid, nid2) == 0 ||
        strcmp(nid2, tempnid2) == 0 && members[i].id == id2) {
      found = 1;
  ...
  if (toupper(yn) == 'Y') {
    ...
    for (int i = 0; i < FSIZE - 1; i++) {
      for (int j = 0; j < FSIZE - 1 - i; j++) {
        ...
        } else if (toupper(choice) == 'D' && members[j].department &&
                   members[j + 1].department) {
          if (strcmp(members[j].department, members[j + 1].department) > 0) {
            swapFaculty(&members[j], &members[j + 1]);
          }
        } else if (toupper(choice) == 'R' && members[j].rank &&
                   members[j + 1].rank) {
